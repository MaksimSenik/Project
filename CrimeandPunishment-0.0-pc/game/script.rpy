...
label start:
    ...
    menu:
        "Заложить часы":
            jump lomb
        "Постараться прожить так":
            s "Раскольникова мучает голод и он решается всё таки заложить часы"
label lomb:
    s "Поэтому решил заложить часы."
    scene out
    with Dissolve(0.5)
    play sound "zahlopnuli_dver_v_komnatu.mp3"
    show rask_s:
        xalign 0.0
        yalign 1.0
    play sound "tolko_shagi.mp3"
    ...
    s "Мылси радеона перебивают звуки ссоры"
    menu:
        "Обернуться":
            jump marm
        "Проигнорировать":
            s "Ссора становится ещё громче и Раскольников инстиктивно оборачивается"
            jump marm

label marm:
    play sound "ssora_sosedei.mp3"
    m2 "ДА ТЫ НИКЧЁМНЫЙ!"
    ...
    r "Что?Где я?"
    play sound "goodmorning.mp3"
    r "Фух…Я всего то задремал."

    v1 "Да, моя сестра процентщица."
    play sound "zvuk_lyudnoy_ulitsyi.wav"
    ...
    s "Наш “Герой” отправился домой и уже поднимаясь к себе домой, он услышал разговор офицера с другим студентом."
    play sound "shagi_medlennyie_v_komnate.mp3"
    menu:
        "Прислушаться":
            jump rasg
        "Поспешить":
